# forex-news_bot v2 — EMA50x100 → EMA9x20 + RSI (H1 only)

บอทนี้แทนที่ระบบเดิมทั้งหมด ด้วยเงื่อนไขใหม่ ทำงานบนกราฟ **H1 เท่านั้น**:
...
ครบทั้ง 3 เงื่อนไข (AND ทั้งหมด) → ส่ง LINE ทันที ข้อความสั้นมาก ประหยัดโควต้า เช่น
...
lib/dataFetcher.js       ดึงแท่งเทียน H1 จาก Yahoo Finance (ตัดแท่งที่ยังไม่ปิดทิ้งเสมอ)
...
7. ตั้ง cron ที่ cron-job.org ให้ยิง URL เดิม ไปที่ `/api/scan?secret=...` — เป็นสัญญาณ H1 ไม่จำเป็นต้องยิงถี่ทุกนาทีเหมือนตอน M1 แล้ว **ทุก 5 นาทีก็เพียงพอ** (แท่ง H1 ใหม่จะปิดทุกชั่วโมงพอดี ยิงถี่กว่านั้นแค่เปลืองโควต้าฟรีของ cron-job.org/Vercel โดยไม่ได้ประโยชน์เพิ่ม — ถ้ามี cron เดิมตั้งไว้ทุก 1 นาที แนะนำเข้าไปแก้ schedule เป็นทุก 5 นาที)

## หมายเหตุสำคัญ

- ทุกอย่างอิงกราฟ **H1 เท่านั้น** ตามที่ระบุ ไม่มีการดูไทม์เฟรมอื่นเลย
